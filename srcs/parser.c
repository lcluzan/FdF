...
/*   Updated: 2024/10/24 18:15:17 by lcluzan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// we reverse the row value cause' in classic geometry, as the y axis goes up, the y value increase, but in such
// a map it's the opposite, the line count increase by going down in the map file.
...
/*
  This function will read a line from the map file, parse it and add it to the chained list of lines.
  It will also check if the width of the map is consistent. If not, it will exit the program.
*/
...
/*
  This function receives a map file path, opens it and parse its content to return a `map` struct.
  It will also compute a scale and offset value.

  the smaller the map is, the bigger the scale will be. This value will be used later in the `draw.c` file to
  zoom in the map and make it fill the screen. So it's computed using the screen size, and map size.

  The offset will be used like the scale value to shift the map coords to the center of the screen.
  The way we parse the map, the origin is in the top left corner, so we'll shift the coords to have the origin
  in the middle of the map.

  Also we use a chained list to avoid having to read the map file twice. A first time to get the height of the map
  by reading each line, then allocate the right size for the map, and then read it a second time to fill this map.
...
// also save the map name as the map path. It will be used as the title of our program window.
